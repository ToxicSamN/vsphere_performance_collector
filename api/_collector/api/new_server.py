    ...
    def start(self):
        ...
        if not self.existing_server:
            ...
            if self.group:
                ...
                # range needs to be definded as totalgroups +1 as the starting number and the end need to be +1
                #    because we are hard coding the group size as +1 and we need to go +1 more than that due to how
                #    range works.
                # for x in range(int(self.totalgroups[0].totalgroups) + 1, int(self.totalgroups[0].totalgroups) + 5):
                #     group_info = group_info + str(x) + ':'
                group_info = str(int(self.totalgroups[0].totalgroups) + 1)

                gmo = Group.objects.create(uuid=self.uuid, group=group_info)
                self.group = Group.objects.filter(uuid=self.uuid)

                tmo = TotalGroups.objects.update(id=1, totalgroups=str(int(self.totalgroups[0].totalgroups) + 1))
                self.totalgroups = TotalGroups.objects.filter(id=1)

                cmo = Collector.objects.create(uuid=self.uuid, hostname=self.hostname, role=self.role, ip=self.ip)
                self.existing_server = Collector.objects.filter(uuid=self.uuid)
