...
class CimSession(requests.Session):
    ...
    def get_session(self):
        response = self.post('https://{}/rest/com/vmware/cis/session'.format(self.vcenter))
        self.headers.update({'vmware-api-session-id': json.loads(response.content.decode('utf8'))['value']})
        return self
